...
def modeling_reg2(train_df, X_train, y_train, test_df, X_test, y_test, class_list):
    #import pandas
    import pandas as pd
    from sklearn.metrics import mean_squared_error
    # create baseline for train_df
    train_df['baseline'] = y_train.mean()
    # create lists for scores
    baseline_rmse = []
    train_score = []
    test_score = []
    ...
    for classifier in class_list:
        ...
        classifier.fit(X_train, y_train)
        # add rmse
        baseline_rmse.append(mean_squared_error(y_train, train_df['baseline'], squared=False))
        rmse.append(mean_squared_error(y_true=y_test, y_pred=classifier.predict(X_test), squared=False))
        #add r2
        train_score.append(classifier.score(X_train, y_train))
        test_score.append(classifier.score(X_test, y_test))
    # combine into dataframe
